...
function Hover{
    set compensation_value to VerticalSpeed.
    if VerticalSpeed > 0.3 {
      Print "///// Hover-> Ascending - Waiting for negative vertical velocity".
      Set throttle_gravity_neutral_vacuum to 0.
    }
    else if VerticalSpeed < -0.1 {
      Print "///// Hover-> Descending - Adding Velocity to Throttle /////".
      Set throttle_gravity_neutral_vacuum to throttle_gravity_neutral_vacuum + abs(Verticalspeed/ship_max_acceleration).
    }
    else {
      Print "///// Hover-> Neutral -  /////".
    }
