...
function hohmann {
  ...
  until false {
    ...
    if T > Tmax {
      ...
      print "eta is in the past".
      set T to T + dt.
    } else if abs(eta) > 1 {
      print "eta is too far".
      set T to T + eta / 2.
    } else {
...
function get_hoffman_node{
  ...
  if ship:obt:eccentricity > 0.1 {
    print "Node Eccentric ship e=" + round(ship:obt:eccentricity, 1).
  }
  if target:obt:eccentricity > 0.1 {
    print "Node Eccentric target e=" +  + round(target:obt:eccentricity, 1).
  }

  global node_ri is obt:inclination - target:obt:inclination.
  if abs(node_ri) > 0.2 {
    print "Node Bad alignment ri=" + round(node_ri, 1).
  }
